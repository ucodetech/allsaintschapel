...
public function getDepartment()
{
  ...
  if ($dept->count()) {
    ...
    return false;
     ...
     {
         $sch = $this->_db->get('schools', array('deleted', '=', 0));
         if ($sch->count()){
             return $sch->results();
         }else {
             return false;
...
public function fetchMembers($val){
         $output = '';
         $imgPath = 'profile/';


         $this->_db->get('members', array('approved', '=', $val));
         if ($this->_db->count()) {
             $dat = $this->_db->results();

             $output .= '
      <table class="table table-striped table-hover" id="ShownewMembers">
        <thead>
          <tr>
            <th>#</th>
            <th>Profile</th>
...
public function fetchMembersApproved(){
         $output = '';
         $imgPath = URLROOT.'chapel_Members/profile/';


         $this->_db->get('members', array('approved', '=', 1));
         if ($this->_db->count()) {
             $dat = $this->_db->results();

             $output .= '
      <table class="table table-striped table-hover" id="ShowMembersApproved">
        <thead>
          <tr>
            <th>#</th>
            <th>Profile</th>
...
public function approveMember($approveid){
      $one = 1;
      $this->_db->update('members','id',$approveid, array(
         'approved' => $one
      ));
      return true;
    }
...
public function getMemberDetail($detail_id){
      $data = $this->_db->get('members', array('id','=',$detail_id));
      if ($data->count()){
          return $data->first();
      }else{
          return false;
...
public function updateMember($memberId,$fields = array()){
      $this->_db->update('members','id', $memberId,$fields);
